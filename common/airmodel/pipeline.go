...
func (this *Pipeline) Build() (string, error) {
	...
	this.properties.Add(
		...
		this.dataSource.GetProperties(),
		this.dataSource.GetRawProperties(),
		nil,
		this.dataSource.GetRuntimeProperties())
...
func (this *Pipeline) buildFlow(flows []Logic) ([]interface{}, error) {
	...
	for index, logic := range flows {
		...
		*/
		...
		log.Info("(Pipeline.buildFlow) Build logic.GetID() : ", logic.GetID(), ", logic.GetProperties() : ", logic.GetProperties())
		this.properties.Add(
			logic.GetID(),
			logic.GetProperties(),
